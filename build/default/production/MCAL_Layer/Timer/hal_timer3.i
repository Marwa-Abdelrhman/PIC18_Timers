...
# 8 "MCAL_Layer/Timer/hal_timer3.c" 2
...
static uint16 timer3Preload_val;
  ...
  static void (* TIMER3_Set_Interrupt_Handler)(void) =((void*)0);
...
Std_RetrunType TIMER3_Init(const timer3_t *tmr3_obj){
    ...
    {
       (T3CONbits.TMR3ON=0);
       (T3CONbits.T3CKPS=tmr3_obj->prescaler_value);
       TMR3H=(tmr3_obj->timer3_preload_val)>>8;
       TMR3L=(uint8)(tmr3_obj->timer3_preload_val);
       timer3Preload_val=tmr3_obj->timer3_preload_val;

       if(tmr3_obj->timer3_mode == 0){
           (T3CONbits.TMR3CS=0);
       }
       else if(tmr3_obj->timer3_mode == 1){
           (T3CONbits.TMR3CS=1);
            if(tmr3_obj->counter_mode == 0){
                 (T3CONbits.T3SYNC=0);
            }
            else if(tmr3_obj->counter_mode == 1){
                 (T3CONbits.T3SYNC=1);
            }

       }

       if(tmr3_obj->timer3_reg_size == 1){
            (T3CONbits.RD16=1);
       }
       else if(tmr3_obj->timer3_reg_size == 0){
            (T3CONbits.RD16=0);
       }
     ...
     (PIE2bits.TMR3IE=1);
     (PIR2bits.TMR3IF=0);
# 72 "MCAL_Layer/Timer/hal_timer3.c"
 (INTCONbits.GIE=1);
 (INTCONbits.PEIE=1);
...
Std_RetrunType TIMER3_read_value(const timer3_t *tmr3_obj,uint16 *tmr3_val){
    Std_RetrunType ret = ((Std_RetrunType) 0x00);
    uint8 l_timer3_L=0,l_timer3_H=0;

    if(tmr3_obj == ((void*)0) || tmr3_val==((void*)0) )
    {
        ret=((Std_RetrunType) 0x00);
    }
    else
