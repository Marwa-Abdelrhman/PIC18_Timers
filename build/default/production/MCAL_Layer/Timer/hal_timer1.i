...
static uint16 timer1Preload_val;
  ...
  static void (* TIMER1_Set_Interrupt_Handler)(void) =((void*)0);
...
Std_RetrunType TIMER1_Init(const timer1_t *tmr1_obj){
    ...
    {
       (T1CONbits.TMR1ON=0);
       (T1CONbits.T1CKPS=tmr1_obj->prescaler_value);
       TMR1H=(tmr1_obj->timer1_preload_val)>>8;
       TMR1L=(uint8)(tmr1_obj->timer1_preload_val);
       timer1Preload_val=tmr1_obj->timer1_preload_val;

       if(tmr1_obj->timer1_mode == 0){
           (T1CONbits.TMR1CS=0);
       }
       else if(tmr1_obj->timer1_mode == 1){
           (T1CONbits.TMR1CS=1);
            if(tmr1_obj->counter_clk == 0){
                 (T1CONbits.T1SYNC=0);
            }
            else if(tmr1_obj->counter_clk == 1){
                 (T1CONbits.T1SYNC=1);
            }

       }

       if(tmr1_obj->timer1_reg_size == 1){
            (T1CONbits.RD16=1);
       }
       else if(tmr1_obj->timer1_reg_size == 0){
            (T1CONbits.RD16=0);
       }
       if(tmr1_obj->timer1_osc_cfg == 1){
           (T1CONbits.T1OSCEN=1);
       }
       else if(tmr1_obj->timer1_osc_cfg == 0){
           (T1CONbits.T1OSCEN=0);
       }
     ...
     (PIE1bits.TMR1IE=1);
     (PIR1bits.TMR1IF=0);
# 77 "MCAL_Layer/Timer/hal_timer1.c"
 (INTCONbits.GIE=1);
 (INTCONbits.PEIE=1);
...
Std_RetrunType TIMER1_read_value(const timer1_t *tmr1_obj,uint16 *tmr1_val){
    Std_RetrunType ret = ((Std_RetrunType) 0x00);
    uint8 l_timer1_L=0,l_timer1_H=0;

    if(tmr1_obj == ((void*)0) || tmr1_val==((void*)0) )
    {
        ret=((Std_RetrunType) 0x00);
    }
    else
...
# 142 "MCAL_Layer/Timer/hal_timer1.c"
Std_RetrunType TIMER1_write(const timer1_t *tmr1_obj,uint16 tmr1_val){
    Std_RetrunType ret = ((Std_RetrunType) 0x00);
    if(tmr1_obj == ((void*)0) )
    {
